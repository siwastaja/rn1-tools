/*
	...
	Tool for reflashing PULUTOF1 Master and RobotBoard Rev.2
...
int main(int argc, char** argv)
{
	...
	if(argc >= 3)
	{
		...
		int size = fread(fw, 1, 256*1024, bin);

		if(size < 512 || size >= 256*1024-4)
		{
		...
		{
			struct __attribute__((packed)) write_cmd { uint32_t magic; uint32_t size; uint8_t crc;} write_cmd;
		...
		{
			printf("Sending the firmware, chunk %u...\n", chunk_count);
			int amount_to_send = size; if(amount_to_send > 65532) amount_to_send = 65532;
			struct spi_ioc_transfer xfer;
			memset(&xfer, 0, sizeof(xfer)); // unused fields need to be initialized zero.
		...
		while(bytes_read < size)
		{
			...
			int amount_to_read = size; if(amount_to_read > 65532) amount_to_read = 65532;
			{
				struct spi_ioc_transfer xfer;
				memset(&xfer, 0, sizeof(xfer));
				xfer.tx_buf = NULL;
		...
		printf("Verification done: %d errors in %d bytes\n", errors, size);
	...
	if(verbose) printf("Everything's done.\n");
